...
icons:
  <clave>:               # p.ej. ward.icon — ojo: Bukkit anida por puntos,
    ...
    hide-name: true      # oculta el nombre del ítem en el tooltip (opcional;
                         #   usada por los botones de menú)
...
| `nucleus.icon` | Nucleus | ítem núcleo (BEACON, CMD 41002, modelo cubo 3D en `beacon.json`) |
| `estate.icon`, `estate.adventure`, `estate.instance`, `estate.dragon`, `estate.zone-tp` | Estate | lobby/instancia |
| `menu.deposit`, `menu.filler`, `menu.back` | genérico | slots comunes |
| `menu.profile`, `menu.close` | genérico | botones de menú (PAPER, CMD 41403/41404, con `hide-name`) |
| `sounds.menu.click / menu.error / menu.success` | genérico | feedback de acciones |
| `fonts.dc.gui` | genérico | fondo de menús dibujado con glifos (`dreamcraft:gui`, ver graphic-assets §6) |
| `fonts.dc.gothic` | genérico | fuente gótica decorativa (`dreamcraft:gothic`, ver graphic-assets §5) |
| `symbols.menu.bg.*` | genérico | glifo de fondo por tamaño de inventario (`\uE100`–`\uE105`) |
| `symbols.menu.offset.*` | genérico | anclaje del fondo: el compositor antepone 2x `\uEC04` (advance -4) para llevar el glifo de 176 px a x=0; el resto queda reservado |
